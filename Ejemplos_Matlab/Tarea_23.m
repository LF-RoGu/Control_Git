...
%dx4_op1 = 2*X(:,1) + 0*X(:,2) + 1*X(:,3) - 2*X(:,4) - U;
    %Op2 2 bloques
        % A x1 lo controlamos con x2
            % A x2 lo controlamos con U
% dx1_op2 = -X(:,1) + 0*X(:,2) - 4*X(:,3) + 3*X(:,4);
% dx2_op2 = 3*X(:,1) - 1*X(:,2) + 2*X(:,3) - 1*X(:,4) + 3*U;
% dx3_op2 = 0*X(:,1) - 2*X(:,2) + 0*X(:,3) - 1*X(:,4);
% dx4_op2 = 2*X(:,1) + 0*X(:,2) + 1*X(:,3) - 2*X(:,4);
...
%Ley de Control
U = [zeros(numel(X(:,1)), 1), -dx4_ref + 2*X(:,1) + 0*X(:,2) + 1*X(:,3) - 2*X(:,4) + k2*e2];
